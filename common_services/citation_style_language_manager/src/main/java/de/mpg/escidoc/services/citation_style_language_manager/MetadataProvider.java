...
{
    private final static Logger logger = Logger.getLogger(MetadataProvider.class);
    ...
    public CSLItemData retrieveItem(String id)
    {
        ...
        try
        {
            ...
            for (CreatorVO creator : metadata.getCreators())
            {
                ...
                {
                    editorList.add(new CSLNameBuilder().given("").family(creator.getOrganization().getName().getValue())
                            .build()); // empty String needed
                }
            }
            ...
            else if (metadata.getDateAccepted() != null && Genre.THESIS.equals(metadata.getGenre())) // Published online
            {
                for (String formatString : dateFormats)
                {
                    try
            ...
            if (metadata.getSources() != null && !metadata.getSources().isEmpty())
            {
                ...
                else
                {
                    ...
                    if (metadata.getSources().size() > 1 && metadata.getSources().get(1) != null)
                    {
                        ...
                        {
                            // Source title
                            cslItem.collectionTitle(secondSource.getTitle().getValue());
                            // Source creators
                            List<CSLName> collectionEditorList = new ArrayList<CSLName>();
                ...
                for (TextVO sourceAlternativeTitle : source.getAlternativeTitles())
                {
                    if (!SourceVO.AlternativeTitleType.HTML.toString().equals(sourceAlternativeTitle.getType())
                            ...
                            && !SourceVO.AlternativeTitleType.MATHML.toString()
                                    .equals(sourceAlternativeTitle.getType()))
                    {
    ...
    private CSLType getCslGenre(Genre genre)
    {
        ...
        else if (Genre.EDITORIAL.equals(genre) || Genre.PAPER.equals(genre) || Genre.OTHER.equals(genre))
        {
            cslGenre = CSLType.ARTICLE;
                ...
                || Genre.ENCYCLOPEDIA.equals(genre) || Genre.FESTSCHRIFT.equals(genre) || Genre.HANDBOOK.equals(genre)
                || Genre.ISSUE.equals(genre) || Genre.JOURNAL.equals(genre) || Genre.MANUAL.equals(genre)
                || Genre.MONOGRAPH.equals(genre) || Genre.MULTI_VOLUME.equals(genre) || Genre.NEWSPAPER.equals(genre)
                || Genre.PROCEEDINGS.equals(genre) || Genre.SERIES.equals(genre))
        {
