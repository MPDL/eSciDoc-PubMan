...
<!-- ***********************************************SOURCE TEMPLATES ***************************************************************** -->	
				...
				<xsl:variable name="coneCreator">
					<xsl:choose>
						...
						<xsl:when test="$import-name = 'MPIL'">
							<xsl:copy-of select="Util:queryConeExact('persons', concat($creatornfamily, ', ', $creatorngiven), 'Max Planck Institute for the Science of Light')"/>
						</xsl:when>
						<xsl:when test="$import-name = 'MPIBF'">
	...
	<xsl:template name="dcTermsSubject">
		...
		<!-- Mapping f. MPI PKS: Collection-Name wird zu MPIPKS-Klassifikation ; Auskommentiert v. Erndt (25.02.16) für nachträgl. Teilimport durch Andrea
		<xsl:if test="$import-name = 'MPIPKS'">
